...
submodule revisions below were refreshed on 2026-07-31:

| Dependency | Revision | Update policy |
| --- | --- | --- |
| [llama.cpp](https://github.com/ggml-org/llama.cpp) | `000547513f1530346ecd163db8b3e13962949961` | Track `master`; the current mtmd/common APIs are adapted by `zz_compat.patch`. |
...
- the `common_sampler_sample2` entry point used by llama-box;
- the `defrag_thold` passthrough and current target/API compatibility.
...
2. Run the CPU configure/build and verify that every patch applies.
